...
多言語対応（日本語・英語）で、管理者承認制のオンラインランキング機能を備えています。
...
```
                    ...
                    静的ファイル  │          │  /api/*
                          ...
                          └──────────┘  └───┬───────┬───┘
                                ...
                                session     │       │  score       │
                                       ...
                                       └───────┘ └──┬──┬──┘  └───────────────┘
                                                     │  │
                                              ┌──────▼┐ │
                                              │DynamoDB│ │  承認待ち (pending:*)
                                              │(top100)│ │  を DynamoDB に保存
                                              └────────┘ │
                                                         │
                               ランクイン時               │
                                    │                    │
                         ...
                         ボタンをクリック           
                                    │
                               ┌────▼────┐
                               │Lambda   │ → 承認: leaderboard 更新 → S3 書き出し
                               │Admin    │ → 削除: pending から削除
                               └─────────┘
```
...
ハイスコアがランクインすると、原則として管理者による承認を経て初めて掲載されます。ただし、既にランキングに掲載されている名前と完全一致する場合は自動承認されます。承認は「名義の承認」であり、特定のスコアに対する承認ではありません。

**通常フロー（新規名）**

1. ユーザーがスコアを送信すると、DynamoDB に承認待ち（`pk: "pending:<name>"`）として名前単位で保存されます
2. 同じ名前で再送信した場合、新しいタイムが速ければ承認待ちのベストスコアが更新されます（遅ければ更新されません）
3. 管理者にメール通知が届きます。メールには「✅ 掲載承認」「❌ 削除」ボタンが含まれます
4. 同一名義のどの通知メールからでも承認・削除操作が可能です（リンクは名前ベースの署名）
5. 承認された場合、承認待ちに残っているベストスコアが `leaderboard.json` に反映されます
6. 既に承認済みの名前を再度承認しようとすると「既に承認済みです」と表示されます
7. 承認後でも削除ボタンをクリックするとランキングから削除されます（監査ログは保持）

承認待ちエントリは 30 日後に DynamoDB の TTL で自動削除されます。

**自動承認フロー（既存名と一致）**

送信された名前がランキング上の既存エントリの名前と完全一致する場合、信頼済みの名前とみなし承認をスキップします。同一名義のエントリはランキングに一つだけ保持され、新しいタイムが既存より速い場合のみ記録が更新されます。既存の方が速い場合は更新されず、通知メールも送信されません。

1. 既存の記録より速い場合、ランキングを直接更新し、`leaderboard.json` を S3 に書き出します
2. 管理者には自動承認済みの通知メールが送信されます（アクションボタンなし）
3. フロントエンドのランキング表示が即時更新されます
...
**ランクイン通知メール（承認/却下ボタン付き）**

ハイスコアがランクインした際に、管理者宛に HTML メールを送信します。メールにはランクイン者の情報、クライアント情報の全項目、最新の順位表に加え、「掲載承認」「削除」ボタンが含まれます。ボタンのリンクには HMAC 署名が付与されており、署名が一致しないリクエストは拒否されます。SES（Amazon Simple Email Service）を使用し、送信の失敗はスコア登録処理に影響しません。

**自演検知（名前を変えた連投の警告）**

監査ログをもとに、**類似 IP（同一サブネット。IPv4 は /24、IPv6 は /64）かつ同一 User-Agent** からの送信を集計し、直近 24 時間で **異なる名前が 2 種類以上**使われている場合、名前を変えた自演・連投の疑いとして管理者通知メールで警告します。同一名義での複数回送信（記録更新などの正当な再挑戦）は警告対象外です。警告はスマートフォンの Gmail 一覧で開かずに気付けるよう、**メール件名の先頭**（`🚨要確認(24h内 別名◯件) …`）と**本文の最上部（赤いバナー）**の両方に表示され、該当送信の名前・送信日時（JST）・IP の一覧を添えます。IP または User-Agent が判定できない場合は誤検知を避けるため警告しません。検知の時間窓（既定 24 時間）と名前種類数のしきい値（既定 2 種類）は `SUSPICIOUS_WINDOW_HOURS` / `SUSPICIOUS_THRESHOLD` で調整できます。

**管理操作の署名検証**

メール内の承認/却下リンクには `AdminSecret` から生成した HMAC-SHA256 署名が含まれます。Admin Lambda はリクエスト受信時に署名を検証し、不正なリクエストを拒否します。

### 順位ロジック

ランキングへの登録は満点（10問全問正解）のみが対象です。満点達成者の中で合計タイムの昇順（速い方が上位）で順位を決定します。上位 100 位以内に入った場合のみ名前の入力を受け付けます。同一名義のエントリはランキングに一つだけ保持され、記録更新時のみ上書きされます。

**ランキング表示のティア**

ランキング表示は順位帯ごとに視覚的に差別化されています。

* 1〜3位 — 🥇🥈🥉のメダル絵文字付き、ボールド、順位ごとに異なるフォントサイズで強調
* 4〜10位 — 通常のテーブル表示
* 11〜20位 — 小さめのフォント、行の縦幅を詰めたコンパクト表示
* 21位以降 — さらに小さいフォントで改行なしのインライン形式（`21: ○○ (**.**びょう) / 22: ○○ (**.**びょう)` の繰り返し）
...
- `AdminSecret` — 管理操作用の署名鍵（32 文字以上のランダム文字列）
...
# 4. ランキングJSON を初期化
echo '{"updatedAt":"","entries":[]}' | aws s3 cp - s3://YOUR_SITE_BUCKET/leaderboard.json \
...
| `AdminSecret` | 管理操作用 HMAC 署名鍵（32 文字以上） | `openssl rand -hex 32` で生成 |
