...
ハイスコアがランクインすると、原則として管理者による承認を経て初めて掲載されます。ただし、既にランキングに掲載されている名前と完全一致する場合は自動承認されます。

**通常フロー（新規名）**

1. ユーザーがスコアを送信すると、DynamoDB に承認待ち（`pk: "pending:*"`）として保存されます
2. 管理者にメール通知が届きます。メールには「✅ 掲載承認」「❌ 削除」ボタンが含まれます
3. 管理者がボタンをクリックすると、Admin Lambda が HMAC 署名を検証した上で処理を実行します
4. 承認された場合のみ `leaderboard.json` が更新され、サイトに反映されます

承認待ちエントリは 30 日後に DynamoDB の TTL で自動削除されます。

**自動承認フロー（既存名と一致）**

送信された名前がランキング上の既存エントリの名前と完全一致する場合、信頼済みの名前とみなし承認をスキップします。

1. ランキングを直接更新し、`leaderboard.json` を S3 に書き出します
